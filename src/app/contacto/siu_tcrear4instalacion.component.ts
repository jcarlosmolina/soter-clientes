...
@Component({
    selector: 'imes-contacto-siu-tcrear4instalacion',
    templateUrl: './siu_tcrear4instalacion.component.html',
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ContactoSIUuTCREAR4INSTALACIONComponent extends AbstractServiceIU implements OnInit {
    /** Fields */
    public pInstalacion: FieldOVLenseSearch;
    ...
    constructor(
        ...
        this.idXML = 'Clas_1699352150016154Ser_5UIServ_1';
        this.className = ModelConstants.Contacto.name;
        this.iuName = ModelConstants.Contacto.siuutcrear4instalacion;
        this.setUserVisibility(this.appGlobalInfo.getLoggedUserInfo().isIUVisible(this.idXML, ['Cliente']));
        this.serviceName = 'TCREAR4INSTALACION';
        this.title = this.langMng.translateText('cls_Contacto_svc_TCREAR4INSTALACION',
            'Alta');
    }
    ...
    public initializeComponents(): void {
        ...
        this.pInstalacion = new FieldOVLenseSearch(this.langMng);
        this.pInstalacion.nameInRequest = 'pinstalacion';
        this.pInstalacion.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pInstalacion',
            'Instalación');
        ...
        this.pTipoContacto.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pTipoContacto',
            'Tipo');
        ...
        this.pNombre.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pNombre',
            'Nombre');
        ...
        this.pTelefono1.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pTelefono1',
            'Teléfono 1');
        ...
        this.pTelefono2.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pTelefono2',
            'Teléfono 2');
        ...
        this.pFax.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pFax',
            'Fax');
        ...
        this.pEmail.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pEmail',
            'Email');
        ...
        this.pRecibePresupuestos.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pRecibePresupuestos',
            'Recibe presupuestos');
        ...
        this.pRecibePartes.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pRecibePartes',
            'Recibe partes');
        ...
        this.pRecibeFacturas.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pRecibeFacturas',
            'Recibe facturas');
        ...
        this.pPuedeFirmar.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pPuedeFirmar',
            'Puede firmar');
        ...
        this.pDNI.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pDNI',
            'DNI');
        ...
        this.pMovil.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pMovil',
            'Móvil');
        ...
        this.pTipoFirma.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pTipoFirma',
            'Tipo firma');
        ...
        this.pIdDispositivo.alias = this.langMng.translateText(
            'cls_Contacto_svc_TCREAR4INSTALACION_inArg_pIdDispositivo',
            'Id. dispositivo');
    ...
    public initializeFromCtxRelated(): void {
        ...
        if (lastRole && lastRoleOid !== {}) {
            if (lastRole === 'Instalacion') {
                ...
                this.executeDependecyRulespinstalacion('SetValue', true);
                this.pInstalacion.enabled = false;
                this.executeDependecyRulespinstalacion('SetEnabled', true);
            }
    ...
    */
    public processSelectionBackward(exchInfo: ExchangeInfo): void {
        ...
        if (exchInfo.originArgumentName.toLowerCase() === 'pInstalacion'.toLowerCase()) {
            this.pInstalacion.addOids(exchInfo.selectedOids);
            this.executeDependecyRulespinstalacion('SetValue', false);
        }

    }

    /**
     * Evaluate dependendy rules for pInstalacion argument
     * @param event Rule event
     * @param internal Internal or External
     */
    private executeDependecyRulespinstalacion(event: string, internal: boolean): void {
        // Suplementary information
        if ('SetValue' === event) {
            this.setSuppInfopinstalacion();
        }
        this.executePendingDependencyRules();
    }

    /**
     * Set the supplementary information to the pInstalacion argument
     */
    private setSuppInfopinstalacion(): void {

        const displaySetIC = 'Nombre';
        let numberMissingSupInfo = 0;
        for (const value of this.pInstalacion.getSelectedInstances()) {
    ...
    /**
     * Process the change in the pInstalacion lense search argument
     */
    public onpinstalacionChange(): void {

        const searchText = this.pInstalacion.text;
        if (searchText && searchText !== '') {
            if (searchText !== this.pInstalacion.lastText) {
                this.pInstalacion.removeCSSClass('noResultsLenseSearch');
                this.pInstalacion.lastText = searchText;
                this.util.searchLenseSearch(ModelConstants.Instalacion.name, 'DSS_Instalacion', searchText, [], null).then(
                        (response: any) => {
                            this.pInstalacion.removeCSSClass('noResultsLenseSearch');
                            this.pInstalacion.searchResults = response.results;
                            if (this.pInstalacion.searchResults &&
                                this.pInstalacion.searchResults.length === 0) {
                                this.pInstalacion.addCSSClass('noResultsLenseSearch');
                            }
                            if (this.pInstalacion.searchResults &&
                                this.pInstalacion.searchResults.length === 1) {
                                this.processpinstalacionSelected(0);
                            }
                            this.changeDetectorRef.markForCheck();
                        },
                        (response: any) => {
                            this.pInstalacion.removeCSSClass('noResultsLenseSearch');
                            this.pInstalacion.searchResults = [];
    ...
    /**
     * Process the selection in the pInstalacion lense search argument
     */
    public processpinstalacionSelected(index: number): void {

        let selectedInstance = null;
        if (index !== -1 && this.pInstalacion.searchResults.length > index) {
            selectedInstance = this.pInstalacion.searchResults[index];
        }
        // Clean the result list
        this.pInstalacion.searchResults = [];
        if (selectedInstance) {
            if (!this.pInstalacion.allowMultiSelect) {
                this.pInstalacion.cleanSelectedInstances();
            }
            selectedInstance.oid = JSON.parse(selectedInstance.jsonOID);
            this.pInstalacion.getSelectedInstances().push(selectedInstance);
        }
        this.executeDependecyRulespinstalacion('SetValue', false);
        this.changeDetectorRef.markForCheck();
    }
    ...
    /**
     * Clean the value of the pInstalacion lense search argument
     */
    public cleanpinstalacion(): void {
        this.clearValidationMessages();
        this.pInstalacion.clean();
        this.executeDependecyRulespinstalacion('SetValue', false);
        this.changeDetectorRef.markForCheck();
    }
    ...
    */
    public doSearchAction(argumentName: string): void {
        ...
        if ('pInstalacion'.toLowerCase() === argumentName.toLowerCase()) {
            this.pInstalacion.completeSelectionForwardExchInfo(exchInfo);
